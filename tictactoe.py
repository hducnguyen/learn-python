...
# Define a function to update the board with the player's move
def moveUpdate(playerMoves):
    # Ask the user to enter their move (a number from 1 to 9)
    indexNumber = int(input("Enter your move: "))
    # Find the corresponding position on the board and update it with the player's mark (X or O)
    for i in range(len(board_game)):
        for j in range(len(board_game[i])):
...
for i in range(rounds):
    # Initialise the Tic-Tac-Toe board with a 3x3 dimension
    board_game = [[], [], []]

    # Fill the board with numbers from 1 to 9, which will serve as moves for the players
    counter = 1
    for i in range(3):
        for j in range(3):
    ...
    while True:
        ...
        if checkForWin(board_game) == True:
            ...
            # Append the score for 2 players
            if playerMoves == "X":
